...
void GeometryWarsApp::prepareSettings(cinder::app::AppBasic::Settings *settings)
{
...
void GeometryWarsApp::setup()
{
	...
	mGridShader				= new gl::GlslProg( loadResource( "passthru.vert" ), loadResource( "colorgrid.frag" ) );
	mBlurShader				= new gl::GlslProg( loadResource( "passthru.vert" ), loadResource( "blur.frag" ) );
...
void GeometryWarsApp::mouseDown( MouseEvent event )
{
	...
	for( int i = 0; i < 3; i++ )
		timeline().add( boost::bind( &GeometryWarsApp::showExplosion, this, event.getPos() ), timeline().getCurrentTime() + 0.15 * (float) i );
}
...
void GeometryWarsApp::update()
{
	...
	for( iter = mColorMaskAreas.begin(); iter != mColorMaskAreas.end(); iter++ ) {
		...
		// If the lifetime exceeds the max lifet ime, we queue for removal
...
void GeometryWarsApp::draw()
{
	...
	// If you want to see the distoration mask, do this
